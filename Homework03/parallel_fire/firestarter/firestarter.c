...
int main(int argc, char ** argv) {
    ...
    int i_prob;
    int n_probs=101;
    int do_display=0; // turning off display
    xgraph thegraph;

    // variables added by Josh Bussis for proj03
    int numProcesses = -1, id = -1;
    double * iterations;
    int chunkSize1, chunkSize2, start, stop, remainder = -1;
    double * total_percent_burned;
    ...
    // allocate space for iterations
    iterations = (double *) malloc (n_probs*sizeof(double));
    total_percent_burned = (double *) malloc (n_probs*sizeof(double));
    total_iterations = (double *) malloc (n_probs*sizeof(double));
    ...
    /*
     * For this problem, I will use the chunk method of parallelization
     * Change the inner loop
     */

    // calculate the size of the chunks to process the problem
    chunkSize1 = (int)ceil(((double)n_trials) / numProcesses);
    remainder = n_trials % numProcesses;
    ...
    for (i_prob = 0 ; i_prob < n_probs; i_prob++) {
        ...
        MPI_Reduce(&iterations[i_prob], &total_iterations[i_prob], 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        MPI_Reduce(&percent_burned[i_prob], &total_percent_burned[i_prob], 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        total_iterations[i_prob] /= n_trials;
        total_percent_burned[i_prob] /= n_trials;

        // print output
        if (id == 0) {
            printf("%lf\t%lf\t%lf\n",prob_spread[i_prob],
            total_percent_burned[i_prob], total_iterations[i_prob]);
        }
    }
    ...
    if (do_display==1) {
        ...
        xgraphDraw(&thegraph,n_probs,0,0,1,1,prob_spread,total_percent_burned);
        pause();
    }
    ...
    free(prob_spread);
    free(percent_burned);
    // make sure to free up the space allocated for added arrays (JB)
    free(iterations);
    free(total_percent_burned);
